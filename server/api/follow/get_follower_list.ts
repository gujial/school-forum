...
export default defineEventHandler(async (event) => {
    ...
    try {
        const result =
            await db.sql`SELECT COUNT(follower_id) AS total FROM Follows WHERE following_id = ${userInfo.userId}`;
        ...
        const { rows } =
            await db.sql`select follower_id from Follows where following_id = ${userInfo.userId} LIMIT ${limit} OFFSET ${offset}`;
