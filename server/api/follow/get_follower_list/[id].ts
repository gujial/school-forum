...
export default defineEventHandler(async (event) => {
    ...
    try {
        const result =
            await db.sql`SELECT COUNT(follower_id) AS total FROM Follows WHERE following_id = ${userId}`;
        ...
        const { rows } =
            await db.sql`select follower_id from Follows where following_id = ${userId} LIMIT ${limit} OFFSET ${offset}`;
