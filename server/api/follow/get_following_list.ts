...
export default defineEventHandler(async (event) => {
    ...
    try {
        const result =
            await db.sql`SELECT COUNT(following_id) AS total FROM Follows WHERE follower_id = ${userInfo.userId}`;
        ...
        const { rows } =
            await db.sql`select following_id from Follows where follower_id = ${userInfo.userId} LIMIT ${limit} OFFSET ${offset}`;
