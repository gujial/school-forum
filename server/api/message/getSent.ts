...
export default defineEventHandler(async (event) => {
    ...
    try {
        const result = await db.sql`SELECT COUNT(*) AS total FROM Messages WHERE sender_id = ${userInfo.userId}`
        ...
        const { rows } =
            await db.sql`SELECT * FROM Messages WHERE sender_id = ${userInfo.userId} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`
